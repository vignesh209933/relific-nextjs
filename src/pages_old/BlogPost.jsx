...
import { getPost, getMediaUrl, normaliseMedia } from "../lib/strapi";
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{
          "--tw-prose-body": Colors.Font_3,
          "--tw-prose-headings": Colors.Primary_font,
        }}
      >
        {/* Description/Excerpt */}
        {post.description && (
      ...
      if (item.data?.attributes) {
        ...
        return {
          url: attrs.url?.startsWith("http")
            ? attrs.url
            : `${import.meta.env.VITE_STRAPI_URL}${attrs.url}`,
      ...
      if (item.url) {
        return {
          url: item.url.startsWith("http")
            ? item.url
            : `${import.meta.env.VITE_STRAPI_URL}${item.url}`,
